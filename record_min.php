<?php
$name_page = 'record_min';
include 'header.php';


echo '<h1>'.$lang['TopMin'] . '</h1>';
?>
<title><?php echo $lang['TopMin']; ?></title>
<div class="form">
...
//    (print_r($sum_num));
    ...
    if (isset($_GET['nick']) and $_GET['nick'] != ''){
        ...
        if ($sum_num !=0){
            echo "<div class=\"sum_place center\"> Призыватель " . $info[$summoner_id][0] . ': ' .  $sum_num+1 . ' <a href="record_min.php' . '?'.$urlParams . $page_summoner . '#' .$sum_num+1  . '">&#8658;</a></div>';
        }
...
for ($ctr; $ctr <= $amount*$page; $ctr++){
    ...
    $version .
    '/img/champion/' .
    $ava[$info[$key][$k['min_key']]] .
    '"><div class="fullchampname">' .
    
    $champs_name_arr[$info[$key][$k['min_key']]] .
    '</div></div>' .  
    "</td>" .
    // $value .
