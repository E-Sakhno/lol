<?php  
$name_page = 'record_rank_winrate';

include 'header.php';


echo '<h1>'.$lang['RecWinrate'] . " (" . $lang[$_GET['qu']] . ")" . '</h1>';
?>
<title><?php echo $lang['RecWinrate'] . " (" . $lang[$_GET['qu']] . ")"; ?></title>
<div class="form">
...
//    (print_r($sum_num));
    ...
    if (isset($_GET['nick']) and $_GET['nick'] != ''){
        ...
        if ($sum_num !=0){
            echo "<div class=\"sum_place center\"> Призыватель " . $info[$summoner_id][0] . ': ' .  $sum_num+1 . ' <a href="record_rank_winrate.php' . '?'.$urlParams . $page_summoner . '#' .$sum_num+1  . '">&#8658;</a></div>';
        }
