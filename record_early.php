<?php
$name_page = 'record_early';
include 'header.php';


echo '<h1>'.$lang['TopEarly-h'] . '</h1>';
?>
<title><?php echo $lang['TopEarly-h']; ?></title>
<div class="form">
...
if (isset($_GET['region'])) {
    ...
    foreach ($champs_name_json['data'] as $key => $vale) {
        $champs_name_arr[$vale['key']] = $vale['name'];
    }
...
//    (print_r($sum_num));
    ...
    if (isset($_GET['nick']) and $_GET['nick'] != ''){
        ...
        if ($sum_num !=0){
            echo "<div class=\"sum_place center\"> Призыватель " . $info[$summoner_id][0] . ': ' .  $sum_num+1 . ' <a href="record_early.php' . '?'.$urlParams . $page_summoner . '#' .$sum_num+1  . '">&#8658;</a></div>';
        }
...
<th data-type="number">' . $lang['LastGame'] . '</th>
<th>' . $lang['Champ'] . '</th>
<th>' . $lang['Rang'] . '</th></tr>
</thead>
    <tbody>'
...
for ($ctr; $ctr <= $amount*$page; $ctr++){
    ...
    $info[$key][$k['nick']] .
        ...
        '</td>
        <td class="center">' . 
        gmdate("Y.m.d H:i", $info[$key][$k['early_point']]/1000).
    "</td><td id=\"" . $info[$key][$k['early_key']] . "\">". '<div class="fullchamp">' . 
    "<img id=\"" . $champs_name_arr[$info[$key][$k['early_key']]] ."\"" . "src=\"http://ddragon.leagueoflegends.com/cdn/" .
    $version .
    '/img/champion/' .
    $ava[$info[$key][$k['early_key']]] .
    '"><div class="fullchampname">' .
    
    $champs_name_arr[$info[$key][$k['early_key']]] .
    '</div></div>' .  
    "</td>" .
    // $value .
