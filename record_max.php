<?php
$name_page = 'record_max';
include 'header.php';


echo '<h1>'.$lang['TopMax'] . '</h1>';
?>
<title><?php echo $lang['TopMax']; ?></title>
<div class="form">
...
//    (print_r($sum_num));
    ...
    if (isset($_GET['nick']) and $_GET['nick'] != ''){
        ...
        if ($sum_num !=0){
            echo "<div class=\"sum_place center\"> Призыватель " . $info[$summoner_id][0] . ': ' .  $sum_num+1 . ' <a href="record_max.php' . '?'.$urlParams . $page_summoner . '#' .$sum_num+1  . '">&#8658;</a></div>';
        }
...
for ($ctr; $ctr <= $amount*$page; $ctr++){
    ...
    $info[$key][$k['nick']] .
        ...
        '</td>
        <td class="center">' . 
        number_format($info[$key][$k['max_point']], 0, '', '&nbsp;') .
    "</td><td class=\"" . $info[$key][$k['max_key']] . "\">". '<div class="fullchamp">' . 
