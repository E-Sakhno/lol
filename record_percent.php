<?php
$name_page = 'record_percent';
include 'header.php';


echo '<h1>'.$lang['TopPercent'] . '</h1>';
?>
<title><?php echo $lang['TopPercent']; ?></title>
<div class="form">

    <form action="record_percent.php" method="get" name="form">
    <?php 
...
if (isset($_GET['region'])) {
    ...
    foreach ($info as $key => $row) {
        $summoners[$key] = $row[$k['max_point']]/$row[$k['total']]*100;
    }
...
//    (print_r($sum_num));
    ...
    if (isset($_GET['nick']) and $_GET['nick'] != ''){
        ...
        if ($sum_num !=0){
            echo "<div class=\"sum_place center\"> Призыватель " . $info[$summoner_id][0] . ': ' .  $sum_num+1 . ' <a href="record_percent.php' . '?'.$urlParams . $page_summoner . '#' .$sum_num+1  . '">&#8658;</a></div>';
        }
